 ...
 */
public class EventObject extends Movable implements Comparable<EventObject>, Disposable {
	 ...
	 * If "id" is not set, it will automatically be computed when
	 * the event is put onto a map.<br>
	...
	/**
	 * If an event consumes input, it triggers touch and push events.
	 * The {@link EventHandler} from the other event is called.
	 */
	public boolean consumeInput = false;
	public BlockingBehaviour blockingBehaviour = BlockingBehaviour.BUILDING_LOW;
	/**
	 * The outreach for pushing other events.
	 * Default = 10 pixel
	 ...
	 * @param name
	 * the name of this event. Don't use the name as key. Names may change and they don't need to be unique. 
	 * @param x
	 * the starting x position
	 * @param y
	 * the starting y position
	 */
	public EventObject(String name, String imagePath, int x, int y, MovementHandler moveHandler) {
		this(name, imagePath, x, y, moveHandler, BlockingBehaviour.NONE, Speed.S07_NORMAL);
	}
	/**
	 * Creates a new event.<br>
	 * @param name
	 ...
	 * the starting y position
	 */
	public EventObject(String name, final String imagePath, int x, int y, MovementHandler moveHandler, BlockingBehaviour blockingBehaviour, Speed moveSpeed) {
	 ...
	 * the starting y position
	 */
	public EventObject(String name, TextureRegion region, int x, int y, MovementHandler moveHandler) {
		this(name, region, x, y, moveHandler, BlockingBehaviour.NONE, Speed.S07_NORMAL);
	}
	/**
	 * Creates a new event.<br>
	 * @param name
	 ...
	 * the starting y position
	 */
	public EventObject(String name, TextureRegion region, int x, int y, MovementHandler moveHandler, BlockingBehaviour blockingBehaviour, Speed moveSpeed) {
		image = region;
		visible = true;
	...
	/**
	 * Creates a new event.<br>
	 * For the undocumented parameters see {@link #setAnimationTexture(String, int, int, int, int, boolean)}
	 * @param name
	 * the name of this event. Don't use the name as key. Names may change and they don't need to be unique. 
	 * @param x
	 * the starting x position
	 ...
	 * the starting y position
	 */
	public EventObject(String name, String animationPath, int animationTileWidth, int animationTileHeight, int anzCols, int anzRows, int x, int y) {
		this(name, animationPath, animationTileWidth, animationTileHeight, anzCols, anzRows, x, y, Move4WayAdapter.$());
	}
	/**
	 * Creates a new event.<br>
	 * For the undocumented parameters see {@link #setAnimationTexture(String, int, int, int, int, boolean, boolean)}
	 * @param name
	 * the name of this event. Don't use the name as key. Names may change and they don't need to be unique. 
	 * @param x
	 * the starting x position
	 ...
	 */
	public EventObject(String name, String animationPath, int animationTileWidth, int animationTileHeight, int anzCols, int anzRows, int x, int y, MovementHandler moveHandler) {
		this(name, animationPath, animationTileWidth, animationTileHeight, anzCols, anzRows, x, y, moveHandler, Speed.S07_NORMAL, Direction.N, BlockingBehaviour.PASSES_LOW_BARRIER, true);
	}
	/**
	 * Creates a new event.<br>
	 * For the undocumented parameters see {@link #setAnimationTexture(String, int, int, int, int, boolean, boolean)}
	 ...
	 */
	public EventObject(String name, String animationPath, int animationTileWidth, int animationTileHeight, int anzCols, int anzRows, int x, int y, MovementHandler moveHandler, Speed moveSpeed, Direction startDirection, BlockingBehaviour blockingBehaviour, boolean isAnimationCompressed) {
		addX(x);
		addY(y);
		z=.1f;
		setAnimationTexture(animationPath, animationTileWidth, animationTileHeight, anzCols, anzRows, isAnimationCompressed, true);
	 ...
	 */
	public EventObject(TiledObject object, TiledObjectGroup layer, TileAtlas atlas, TiledMap map) {
		...
		if (object.gid>0) {
			...
			if (prop==null || prop.length()==0) {
				...
				if (z == 0 && prop!=null && prop.length()>0) try {
					z =+ Integer.parseInt(prop);
				} catch (NumberFormatException e) {}
			}
			image = region;
	...
	/**
	 * Centers the touch bounds of this event.
	 * The draw bound is used for centering the touch bound.
	 */
	public void centerTouchbound() {
		touchBound.x = drawBound.x + (drawBound.width-touchBound.width)*.5f;
		touchBound.y = drawBound.y + (drawBound.height-touchBound.height)*.5f;
	}
	/**
	 * Centers the draw bounds of this event.
	 * The touch bound is used for centering the draw bound.
	 */
	public void centerDrawbound() {
		drawBound.x = touchBound.x + (touchBound.width-drawBound.width)*.5f;
		drawBound.y = touchBound.y + (touchBound.height-drawBound.height)*.5f;
	}
	/**
	...
	/**
	 * Compares an event with an other to determine the
	 * rendering order. This method is used by the MapRenderService.
	 * @param o
	 * other event
	...
	/**
	 * Compares an event with a MapRenderRegion to determine the
	 * rendering order. This method is used by the MapRenderService.
	 * @param o
	 * MapRenderRegion
	 * @return rendering order: 1 if this event is in front of the other
	...
	/**
	 * The event will automatically be animated by using this
	 * movement method.
	 */
	@Override
	...
	/**
	 * The event will NOT automatically be animated by using this
	 * movement method. You have to use one of the animate-methods
	...
	public void animate(float x, float y, float deltaTime) {
		if (animation != null) {
			this.image = animation.animate(x,y,Direction.fromMovement(x, y),deltaTime);
		}
	}
	/**
	 * Uses TileAnimation.animate(int, Speed) to set the image for this event.<br>
	...
	public void animate(int animationTextureRow, Speed speed, float deltaTime) {
		if (animation != null) {
			this.image = animation.animate(animationTextureRow, speed, deltaTime);
		}
	}
	/**
	...
	@Override
	public void stop() {
		if (animation != null) this.image = animation.stop();
	}
	/**
	 * ATTENTION: The new animation texture must have exactly the same form as the old one!<br>
	...
	public void changeAnimationTexture(String path) {
		if (animation==null) throw new IllegalStateException("You have to initialize the animation before changing the texture");
		this.image = animation.setAnimationTexture(path);
	}
	/**
	 * Loads a new animation-texture for this event.<br>
	 * For compatibility with old graphic cards,
	 * the texture should have a width and a height by the power of two.<br> 
	 * Good sizes for animation textures are for example: 256x256 pixel, 256x512 pixel
	 * 1024x512 pixel. Don't use textures with more than 2048 pixel width or height.
	 * @param path
	 * The path to the texture file - which should be a power of 2 sized png image
	 * @param tileWidth
	 ...
	 * true, if you want to recompute the touchBound from the new tileWidth and tileHeight.
	 */
	public void setAnimationTexture(String path, int tileWidth, int tileHeight, int anzCols, int anzRows, boolean estimateTouchBound) {
		if (animation==null) {
			animation = new TileAnimation(path, tileWidth, tileHeight, anzCols, anzRows);
			imageOrAnimationAutoDispose = animation;
			image = animation.getActualTextureRegion();
		} else {
			image = animation.setAnimationTexture(path, tileWidth, tileHeight, anzCols, anzRows, false);
		}
		this.drawBound.width = tileWidth;
		this.drawBound.height = tileHeight;
		if (estimateTouchBound) estimateTouchBound();
	}
	/**
	 * Loads a new animation-texture for this event.<br>
	 * For compatibility with old graphic cards,
	 ...
	 * <ul>
	 * <li>If there is only one row, a compressed animation is uncompressed by flipping this row to a second one.<br>
	 * <li>If there are three rows, the third row represents the east direction and will be flipped to the west.<br>
	 * <li>If there are fife rows, the third row represents the east direction and will be flipped to the west.<br>
	 * The fourth row represents the south east direction and will be flipped to the south west.<br>
	 * And the fifth row represents the north east direction and will be flipped to the north west.
	 * </ul>
	 * @param estimateTouchBound
	 * true, if you want to recompute the touchBound from the new tileWidth and tileHeight.
	 */
	public void setAnimationTexture(String path, int tileWidth, int tileHeight, int anzCols, int anzRows, boolean isCompressed, boolean estimateTouchBound) {
		if (animation==null) {
			animation = new TileAnimation(path, tileWidth, tileHeight, anzCols, anzRows, isCompressed);
			imageOrAnimationAutoDispose = animation;
			image = animation.getActualTextureRegion();
		} else {
			image = animation.setAnimationTexture(path, tileWidth, tileHeight, anzCols, anzRows, isCompressed);
		}
		this.drawBound.width = tileWidth;
		this.drawBound.height = tileHeight;
		if (estimateTouchBound) estimateTouchBound();
	...
	/**
	 * Sets a animation for this event. Never use the same animation-object
	 * for two events because the animation will automatically be disposed with the event!<br>
	 * You have to dispose the old animation if you don't need it anymore (simply chain with .dispose()).
	 * The new animation will automatically be disposed.<br>
	 * Be careful when using this method!
	 * @param animation
	 * The animation used for animating this event
	 * @param estimateTouchBound
	 * true, if you want to recompute the touchBound from the new tileWidth and tileHeight.
	 * @return The old animation. If you don't need it any more you have to dispose it!
	 */
	public TileAnimation setAnimation(TileAnimation animation, boolean estimateTouchBound) {
	...
	/**
	 * The animation-object will automatically be disposed with the event!
	 * Never use the same animation-object for two events.
	 * @return The actual running animation.
	 * Never use this inside an other event!
	 */
